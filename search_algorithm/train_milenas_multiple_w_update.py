...
parser.add_argument('--mixed_portion', type=float, default=0.5, help='mixed portion for training data')
...
def main():
    ...
    train_split = int(np.floor(args.mixed_portion * num_train))
    valid_split = int(np.floor((args.mixed_portion + 0.1)*num_train))

    train_inner_train_queue = torch.utils.data.DataLoader(
        train_data, batch_size=args.batch_size * len(gpus),
        sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:train_split]),
        pin_memory=True, num_workers=2)

    train_inner_valid_queue = torch.utils.data.DataLoader(
        train_data, batch_size=args.batch_size * len(gpus),
        sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[train_split:valid_split]),
        pin_memory=True, num_workers=2)

    valid_queue = torch.utils.data.DataLoader(
        train_data, batch_size=args.batch_size * len(gpus),
        sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[valid_split:num_train]),
        pin_memory=True, num_workers=2)
    ...
    for epoch in range(args.epochs):
        ...
        # training
        train_acc, train_obj, train_loss = train(epoch, train_inner_train_queue, train_inner_valid_queue, model, architect, criterion, optimizer, lr)
        ...
        wandb.log({"model_size": mode_size, "epoch": epoch})
